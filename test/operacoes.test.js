...
describe("Suíte de Testes Fraca para 50 Operações Aritméticas", () => {
  ...
  // === Testes de fronteira para matar mutantes sobreviventes ===
  test("51. deve lançar erro ao calcular raiz quadrada de número negativo", () => {
  ...
  test("52. deve tratar fatorial para 0, 1 e números negativos", () => {
    expect(fatorial(0)).toBe(1);
    expect(fatorial(1)).toBe(1);
    expect(() => fatorial(-3)).toThrow(/números negativos/);
  });

  test("53. deve retornar 0 para a média de um array vazio", () => {
    expect(mediaArray([])).toBe(0);
  });

  test("54. deve lançar erro para máximo e mínimo de array vazio", () => {
    expect(() => maximoArray([])).toThrow(/valor máximo/);
    expect(() => minimoArray([])).toThrow(/valor mínimo/);
  });

  test("55. deve validar pares e ímpares nas bordas", () => {
    expect(isPar(3)).toBe(false);
    expect(isImpar(8)).toBe(false);
  });

  test("56. deve detectar números não primos", () => {
    expect(isPrimo(0)).toBe(false);
    expect(isPrimo(1)).toBe(false);
    expect(isPrimo(9)).toBe(false);
  });

  test("57. deve respeitar os limites do clamp", () => {
  ...
  test("58. deve diferenciar divisível de não divisível", () => {
    expect(isDivisivel(10, 2)).toBe(true);
    expect(isDivisivel(10, 3)).toBe(false);
  });

  test("59. deve converter temperaturas com valores não triviais", () => {
    expect(celsiusParaFahrenheit(100)).toBe(212);
    expect(fahrenheitParaCelsius(212)).toBe(100);
  });

  test("60. deve lançar erro ao inverter zero", () => {
    expect(() => inverso(0)).toThrow(/número zero/);
  });

  test("61. deve tratar igualdade nas comparações", () => {
    expect(isMaiorQue(5, 5)).toBe(false);
    expect(isMenorQue(5, 5)).toBe(false);
    expect(isEqual(7, 8)).toBe(false);
  });

  test("62. deve calcular a mediana com array não ordenado e par", () => {
    expect(medianaArray([4, 1, 3, 2])).toBe(2.5);
  });

  test("63. deve lançar erro ao calcular mediana de array vazio", () => {
    expect(() => medianaArray([])).toThrow(/mediana/);
  });

  test("64. deve retornar 1 para o produto de um array vazio", () => {
